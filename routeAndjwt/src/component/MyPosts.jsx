...
const MyPosts = () => {
  ...
  const boardPerPage = 3; // 1 페이지에 5개
  ...
  useEffect(() => {
    ...
    async function fetchBoards() {
      try {
        ...
        if (response.ok) {
          const data = await response.json(); // json 문자열 body 를 js 객체로 변환
          setBoards(data);
        }
      } catch (error) {
    ...
    fetchBoards(); // 함수 실행
  }, []); // [] 의존성 리스트가 비어있으면 처음 렌더링 할 때 한번만 실행됨.
